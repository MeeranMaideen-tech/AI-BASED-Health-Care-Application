...
// const TransferToHospital = ({ patients }) => {
//   return (
//     <div className="p-6">
...
//       </h2>
//       {patients.length === 0 ? (
//         <p className="text-gray-600 dark:text-gray-300">No patients transferred to Hospital.</p>
//       ) : (
//         <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
//           {patients.map((patient) => (
//             <div
//               key={patient.user_id}
//               className="bg-white dark:bg-gray-900 shadow-lg rounded-lg p-5 flex flex-col items-center text-center border border-red-500"
//             >
//               <img
...
//               <span className="mt-3 px-3 py-1 text-sm font-semibold bg-red-100 text-red-700 rounded-full">
//                 Hospital Care
//               </span>
//             </div>
//           ))}
...
// export default TransferToHospital;

import React, { useEffect, useState } from "react";
import axios from "axios";

const TransferToHospital = () => {
  const [patients, setPatients] = useState([]);

  useEffect(() => {
    axios.get("http://localhost:5000/api/user/care/hospital")
      .then(res => setPatients(res.data || []))
      .catch(err => console.error("❌ Failed to fetch hospital-care patients:", err));
  }, []);
  

  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">Transfer to Hospital</h2>

      {patients.length === 0 ? (
        <p className="text-gray-600">No patients transferred to Hospital.</p>
      ) : (
